...
export default function MachineSlider({ machines }: { machines: Machine[] }) {
  ...
  return (
    <div className="relative group/slider">
          ...
          {displayMachines.map((machine) => (
            ...
            >
              ...
              >
                <div className="aspect-[4/3] relative overflow-hidden bg-gray-100">
                  ...
                  {machine.year && machine.year !== 'Årgang ukendt' && (
                    <div className="absolute top-3 left-3">
                      <span className="bg-white/95 backdrop-blur-sm text-[12px] font-semibold px-2.5 py-1 rounded-lg text-[#1a1a1a] shadow-sm">
                        {machine.year}
                      </span>
                    </div>
                  )}
                ...
                <div className="p-5">
                  <div className="mb-2">
                    <span className="text-[11px] text-[#51af37] font-semibold uppercase tracking-wider">
                      {machine.brand}
                    </span>
                  </div>
                  <h3 className="font-semibold text-[#1a1a1a] text-[15px] mb-3 line-clamp-2 leading-snug group-hover:text-[#51af37] transition-colors duration-300">
                    {machine.title}
                  </h3>
                  <div className="flex items-end justify-between pt-3 border-t border-gray-100">
                    ...
                    <span className="text-[13px] font-semibold text-[#51af37] opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      Se mere →
                    </span>
                  </div>
                </div>
      ...
      <button
        onClick={scrollPrev}
        className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 bg-white shadow-xl shadow-black/10 rounded-full p-3 hover:bg-[#1a1a1a] hover:text-white transition-all duration-300 z-10 opacity-0 group-hover/slider:opacity-100 hover:scale-110"
      ...
      <button
        onClick={scrollNext}
        className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-3 bg-white shadow-xl shadow-black/10 rounded-full p-3 hover:bg-[#1a1a1a] hover:text-white transition-all duration-300 z-10 opacity-0 group-hover/slider:opacity-100 hover:scale-110"
