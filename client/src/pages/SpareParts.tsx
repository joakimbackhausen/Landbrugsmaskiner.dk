...
export default function SpareParts() {
  ...
  const sidebarContent = (
    <>
        ...
        <div className="space-y-0.5 max-h-[280px] overflow-y-auto scrollbar-thin pr-1">
          <button
            ...
            className={`w-full text-left px-3 py-2 rounded-lg text-[14px] font-medium transition-colors ${
              !selectedBrand ? 'bg-[#51af37]/10 text-[#1a1a1a] font-semibold' : 'text-gray-600 hover:bg-gray-50'
            }`}
          >
            Alle mærker
          </button>
          {brands.map(b => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-3 py-2 rounded-lg text-[14px] font-medium transition-colors ${
                  selectedBrand === b.brand ? 'bg-[#51af37]/10 text-[#1a1a1a] font-semibold' : 'text-gray-600 hover:bg-gray-50'
                }`}
              >
      ...
      {/* Categories */}
      <CollapsibleSection title="Kategorier" defaultOpen={false}>
        <div className="space-y-0.5">
          <button
            onClick={() => { navigate('/reservedele'); setMobileSidebar(false); }}
            className={`w-full text-left px-3 py-2 rounded-lg text-[14px] font-medium transition-colors ${
              !selectedCategory ? 'bg-[#51af37]/10 text-[#1a1a1a] font-semibold' : 'text-gray-600 hover:bg-gray-50'
            }`}
          >
          ...
          {categoriesWithCounts.map(cat => (
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-lg text-[14px] font-medium transition-colors ${
                selectedCategory === cat.slug ? 'bg-[#51af37]/10 text-[#1a1a1a] font-semibold' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
              <span className="flex justify-between items-center">
                {cat.name}
        ...
        <div className="space-y-0.5">
          {(Object.keys(sortLabels) as SortOption[]).map(key => (
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-lg text-[14px] font-medium transition-colors ${
                sortBy === key ? 'bg-[#51af37]/10 text-[#1a1a1a] font-semibold' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
              {sortLabels[key]}
            </button>
                    ...
                    <div className="p-1.5">
                      {(Object.keys(sortLabels) as SortOption[]).map(key => (
                        <button key={key} onClick={() => { setSortBy(key); setShowSortDrop(false); }}
                          className={`w-full text-left px-3 py-2.5 rounded-md text-[14px] ${sortBy === key ? 'bg-[#51af37]/10 font-semibold text-[#1a1a1a]' : 'hover:bg-gray-50 text-gray-600'}`}>
                          {sortLabels[key]}
                        </button>
                      ))}
                    </div>
        ...
        <div className="max-w-[1260px] mx-auto px-5 sm:px-6 py-6">
          <div className="flex gap-8">
            ...
            <aside className="hidden lg:block w-[260px] flex-shrink-0">
              <div className="sticky bg-white rounded-xl border border-gray-100 p-5" style={{ top: 'calc(var(--header-h, 124px) + 56px)' }}>
                {sidebarContent}
              </div>
            </aside>
            ...
            <div className="flex-1 min-w-0">
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                {filtered.map(part => (
                  <div key={part.id} className="group bg-white rounded-xl overflow-hidden border border-gray-100 hover:shadow-lg transition-all duration-300">
                    ...
                    <div className="p-4">
                      <p className="text-[11px] text-[#51af37] font-semibold tracking-[0.1em] uppercase mb-1.5">{part.brand}</p>
                      ...
                      <div className="flex items-end justify-between pt-3 border-t border-gray-100">
                        ...
                        <a href="tel:+4586379268"
                          className="text-[13px] font-semibold text-[#51af37] hover:text-[#469e2f] flex items-center gap-1 transition-colors">
              ...
              {filtered.length === 0 && (
                <div className="text-center py-20 text-gray-400">
                  ...
                  <button onClick={clearAll}
                    className="text-[#51af37] hover:underline text-[15px] font-medium">Ryd alle filtre</button>
                </div>
              )}
            </div>
          </div>
