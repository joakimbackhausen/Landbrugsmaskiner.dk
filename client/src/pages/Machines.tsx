          ...
          <div className="max-w-[1260px] mx-auto px-5 sm:px-6 py-3.5">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="relative">
                ...
                >
                  Kategori
                  {selectedCatName && <span className="bg-[#51af37] text-white text-[12px] px-2 py-0.5 rounded">{selectedCatName}</span>}
                    ...
                    <div className="p-1.5">
                      ...
                      {categories.map(c => (
                        <button key={c.slug} onClick={() => { navigate(`/maskiner/${c.slug}`); setShowCatDrop(false); }}
                          className={`w-full text-left px-3 py-2.5 rounded-md text-[14px] flex justify-between ${selectedCategory === c.slug ? 'bg-[#51af37] text-white' : 'hover:bg-gray-50'}`}>
                          <span>{c.name}</span>
              ...
              <div className="relative">
                ...
                >
                  Mærke
                  {selectedBrand && <span className="bg-[#51af37] text-white text-[12px] px-2 py-0.5 rounded">{selectedBrand}</span>}
                  <ChevronDown className="w-4 h-4" />
                </button>
                {showBrandDrop && (
                    ...
                    <div className="p-1.5">
                      ...
                      {brands.map(b => (
                        <button key={b.brand} onClick={() => { setSelectedBrand(b.brand); setShowBrandDrop(false); }}
                          className={`w-full text-left px-3 py-2.5 rounded-md text-[14px] flex justify-between ${selectedBrand === b.brand ? 'bg-[#51af37] text-white' : 'hover:bg-gray-50'}`}>
                          <span>{b.brand}</span>
                ...
                {filtered.map(machine => (
                  <Link key={machine.id} href={`/maskine/${machineSlug(machine.id, machine.title)}`} className="group block">
                    <div className="bg-white rounded-xl overflow-hidden border border-gray-100 hover:shadow-lg transition-all duration-300">
                      ...
                      <div className="p-4">
                        <p className="text-[11px] text-[#51af37] font-semibold tracking-[0.1em] uppercase mb-1.5">{machine.brand}</p>
